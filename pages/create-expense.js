...
class CreateExpensePage extends React.Component {
  ...
  componentDidUpdate(oldProps) {
    ...
    if (!oldProps.step !== this.props.step && this.stepTitleRef.current) {
  ...
  onSummarySubmit = async () => {
    try {
      ...
      const result = await this.props.createExpense({
        ...
        expense: {
          ...pick(expense, ['description', 'type', 'privateMessage', 'payoutMethod']),
          ...
          <React.Fragment>
            ...
            <Container position="relative" minHeight={800}>
              ...
              <Box maxWidth={1160} m="0 auto" px={[2, 3, 4]} py={[4, 5]}>
                {step === STEPS.SUMMARY && (
                  <StyledLink color="black.600" onClick={() => this.setState({ step: STEPS.FORM, error: null })}>
                    &larr;&nbsp;
                    <FormattedMessage id="Back" defaultMessage="Back" />
                  </StyledLink>
                )}
                <Flex justifyContent="space-between" flexWrap="wrap">
                  <Box flex="1 1 500px" mt={4} minWidth={300} maxWidth={750} mr={[3, null, 5]}>
                    <H4 mb={24} py={2} ref={this.stepTitleRef}>
                      {step === STEPS.FORM ? (
